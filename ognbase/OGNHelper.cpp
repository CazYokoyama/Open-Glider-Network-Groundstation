...
//const char aprsServer[] = "10.0.0.200";
...
static float SnrCalc(float rssi){
  float noise = -120.0; // -120 dBm BW 200KHz
  return(rssi-(noise));
}
...
static long double distance(long double lat1, long double long1,
...
void OGN_APRS_Export(){
  ...
  String symbol[16] = {"z","^","^","X","","^","g","g","^","^","^","O","^","\'","","n",};
     ...
     for (int i=0; i < MAX_TRACKING_OBJECTS; i++) {
      if (Container[i].addr && (this_moment - Container[i].timestamp) <= EXPORT_EXPIRATION_TIME && Container[i].distance < settings->range){
        ...
        // W = str(lat_m)[5:6] + str(lon_m)[5:6]
        String W_lat = String((Container[i].latitude - int(Container[i].latitude) * 1000)).substring(2, 3);
        String W_lon = String((Container[i].longitude - int(Container[i].longitude) * 1000)).substring(2, 3);
  
        APRS_AIRC.pos_precision = W_lat + W_lon;
